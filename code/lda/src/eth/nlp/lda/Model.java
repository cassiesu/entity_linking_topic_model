...
public class Model {	
	...
	protected boolean readTAssignFile(String tassignFile){
		try {
			...
			String line;
			z = new Vector[M];			
			data = new LDADataset(M);			//一定要存入data中
			data.V = V;			
			for (i = 0; i < M; i++){
	...
	public boolean saveModelTAssign(String filename){
		System.out.println("Save tassign");
	...
	public boolean saveModelTwords(String filename){
		try{
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream(filename), "UTF-8"));					//如果有中文，必须强调编码格式，否则字节数不同
	...
	public boolean initNewModel(LDACmdOption option){
		...
		for (m = 0; m < data.M; m++){
			...
			//initialize for z
			for (n = 0; n < N; n++){
				int topic = (int)Math.floor(Math.random() * K);		//最大整数
				z[m].add(topic);

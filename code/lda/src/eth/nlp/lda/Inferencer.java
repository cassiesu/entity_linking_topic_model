...
public class Inferencer {	
	...
	public boolean init(LDACmdOption option){
		...
		if (!trnModel.initEstimatedModel(option))		//先获取得到已经训练好的trnModel
			return false;		
		
		globalDict = trnModel.data.localDict;		//globalDict是trnModel.data.localDict（trnModel w 与newModel _w之间的联系），与phi的计算相关
		computeTrnTheta();
		computeTrnPhi();

...
def resultfromtopicmodel():
	filename = 'entity_relevence_train_50.txt'
	f1 = open(filename,'r')
	tpresult = dict()
	for line in f1:
	...
	for query in queries:
		...
		if (topicmodelresult.get(text)):
			...
			for annotation in annotations:
				gtdict[annotation.find('span').text] = annotation.find('target').text[29:].replace('_',' ').lower()

			f1relaxsum = f1relaxsum + F1relax(gtdict, predict)
			tagmerelaxsum = tagmerelaxsum + F1relax(gtdict, tagmetest)
			print "f1relax: " +  str(F1relax(gtdict, predict))
			print "tagmef1relax: " + str(F1relax(gtdict, tagmetest))
			f1strictsum = f1strictsum + F1strict(gtdict, predict)
			tagmestrictsum = tagmestrictsum + F1strict(gtdict, tagmetest)
			print "f1strict " +  str(F1strict(gtdict, predict))
			print "tagmef1strict: " + str(F1strict(gtdict, tagmetest))						
			f1num = f1num + 1
		else:
		...
		for key in predict.keys():
			...
			tar.text = 'http://en.wikipedia.org/wiki/'+predict[key].replace(' ','_')

	print "f1strict: " + str(f1strictsum/f1num)
	print "f1relax: " + str(f1relaxsum/f1num)
	print "tagmestrict: " + str(tagmestrictsum/f1num)
	print "tagmerelax: " + str(tagmerelaxsum/f1num)
	print f1num
